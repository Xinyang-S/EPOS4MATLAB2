...
while (trialNum<trials)
    ...
    if (clockStart > clockStartPrev + waitBetweenStims)
        ...
        Motor1.SetWithAnalog(1,3500); % untrigger, analog output pin, analog mV value, max 4000
        Motor1.SetWithDigital(1,16,1); % untrigger, analog output pin, analog mV value, max 4000
...
%     end
    c = clock;
    clockNew = c(4)*3600+c(5)*60+c(6);
    if (clockNew > clockPrev + 2)
        if (high == 1)
            high = 0;             
            Motor1.SetWithDigital(2, high); % untrigger, analog output pin, analog mV value, max 4000
            pause (0.2);
            Motor1.SetWithDigital(1, 0); % untrigger, analog output pin, analog mV value, max 4000
            %disp(high);
        else
            high = 1;
            Motor1.SetWithDigital(2, high); % untrigger, analog output pin, analog mV value, max 4000
            pause (0.2);
            Motor1.SetWithDigital(1, 1); % untrigger, analog output pin, analog mV value, max 4000
            %disp(high);
        end
        clockPrev=clockNew;
    end
