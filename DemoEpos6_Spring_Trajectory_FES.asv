...
% pulsewith (100-400us, by 10), amplitude (1-12 mA safe, by 1), time(us)(1ms -3000ms)
...
%fes activation
writeline(bt,strcat("freq ",num2str(200)));      %Set stim frequency - add a space
cmd = generate_command(elecArray, [0 0 0], [300 300 300], elecname, velecnumber); % Params for start stimulation, elecrtrodes, mA, us pw 
...
while (clockStart < clockStartPrev + hold)
    ...
    %desired trajectory 
    startPositionNew = startPosition + 2000*sin((clockStart-clockBegin)*0.5);

    %calculate distance into wall
    posError = Motor1.ActualPosition - startPositionNew;
   
    %calculate error velocity
    posErrorDiff = posError - posErrorPrev;
    posClockDiff = clockStart - posClockPrev;
   
    %set robot current = KW*distance + DW*Velocity
    ...
    %set FES pulse width propotrional to robot current; keep fes amplitude constant
    pwFES = current*400/10000;
    % if positive use flxors, if negative use extensors
